...
public:
    CProgressWnd();
    CProgressWnd(CWnd* pParent, LPCTSTR pszTitle = _T("Progress"), BOOL bSmooth = TRUE);
    virtual ~CProgressWnd();

    BOOL Create(CWnd* pParent, LPCTSTR pszTitle, BOOL bSmooth = TRUE);
    BOOL GoModal(LPCTSTR pszTitle =_T("Progress"), BOOL bSmooth = TRUE);
...
public:
	...
	void NoCancelButton() { m_CancelButton.EnableWindow(FALSE); m_bNoCancel = TRUE; }
...
class CProgressFunc {
public:
	CProgressFunc(char *caption);
	~CProgressFunc();

	static bool SetPos(int percent);
	static CProgressWnd *pProgressWnd;
};
